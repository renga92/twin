...
function dynamicDoctorButtons() {
  ...
  let locations = all_locations.filter(
    (m, i) => all_locations.indexOf(m) === i
  );
...
// Function to change selection on doctor filter
// based on selected filter
//======================================================

function changeDoctorSelection() {
  $("#doctor-grid-buttons .filter-btn").on("click", function (e) {
    e.preventDefault();
    let doctors = DOCTORS;
...
function dynamicPatientsImages() {
  ...
  patient.map((m, i) => {
    let gridItem = `<div  class='grid-item ${
      i == 0 ? "active" : ""
    }' ><div class="image-bg"><img data-id="${i}" src="${
      m.imagePath
    }"/></div></div>`;
